...
function HeroSection() {
  ...
  useEffect(() => {
    ...
    setIsIOS(/iPad|iPhone|iPod|Mac/.test(navigator.userAgent) && !window.MSStream);
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      ...
      // Fallback for browsers (like Safari/iOS) that do not support beforeinstallprompt
  ...
  return (
    <section id="about-us" style={{ position: "relative", minHeight: "100vh", overflow: "hidden", display: "flex", alignItems: "center", background: "#fafbff" }}>
      ...
      {/* ─── CUSTOM INSTALL MODAL (iOS FALLBACK) ─── */}
      {showInstallModal && (
        <div style={{
          position: "fixed", inset: 0, zIndex: 9999, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)",
          display: "flex", alignItems: "flex-end", justifyContent: "center", padding: "0 0 24px 0",
          animation: "fadeIn 0.2s ease-out"
        }} onClick={() => setShowInstallModal(false)}>
          <div style={{
            background: "white", width: "100%", maxWidth: 400, borderRadius: "24px 24px 20px 20px",
            padding: "32px 24px", boxShadow: "0 -10px 40px rgba(0,0,0,0.15)",
            ...
            <button onClick={() => setShowInstallModal(false)} style={{
              ...
              width: 32, height: 32, borderRadius: 16, fontSize: 16, cursor: "pointer", color: "#555",
              display: "flex", alignItems: "center", justifyContent: "center"
            }}>✕</button>
            <div style={{ width: 48, height: 48, background: "#E8EEFF", color: "#3B5BDB", borderRadius: 16, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, marginBottom: 16 }}>📱</div>
            <h3 style={{ fontFamily: "var(--font-display)", fontSize: 24, fontWeight: 800, color: "#1a1a2e", marginBottom: 8 }}>Install App</h3>

            {isIOS ? (
              <p style={{ fontSize: 15, color: "#555", lineHeight: 1.6, marginBottom: 20 }}>
                Apple restricts direct installations. To install Finova Academy on your iPhone or iPad:
              </p>
            ) : (
              <p style={{ fontSize: 15, color: "#555", lineHeight: 1.6, marginBottom: 20 }}>
                To install Finova Academy for a better experience:
              </p>
            )}

            {isIOS ? (
              <div style={{ background: "#f8f9fa", borderRadius: 16, padding: "16px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, background: "white", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", fontSize: 18 }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path><polyline points="16 6 12 2 8 6"></polyline><line x1="12" y1="2" x2="12" y2="15"></line></svg></div>
                  <div style={{ fontSize: 14, color: "#444", fontWeight: 600 }}>1. Tap the <span style={{ color: "#007AFF" }}>Share</span> icon below</div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ fontSize: 14, color: "#444", fontWeight: 600 }}>2. Select <span style={{ color: "#1a1a2e" }}>Add to Home Screen</span></div>
                </div>
              </div>
            ) : (
              <div style={{ background: "#f8f9fa", borderRadius: 16, padding: "16px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, background: "white", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", fontSize: 18 }}>🖥️</div>
                  <div style={{ fontSize: 14, color: "#444", fontWeight: 600 }}>Look for the <span style={{ color: "#3B5BDB" }}>Install icon</span> in your browser's address bar or menu.</div>
                </div>
              </div>
            )}

            <button onClick={() => setShowInstallModal(false)} style={{
              width: "100%", padding: "14px", marginTop: 24, borderRadius: 14, background: "#1a1a2e", color: "white", border: "none", fontSize: 15, fontWeight: 700, cursor: "pointer"
            }}>Got it</button>
          </div>
        </div>
      )}
